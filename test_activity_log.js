...
async function runTests() {
  ...
  try {
    ...
    const activityItems = await page.evaluate(() => {
      const items = [];
      document.querySelectorAll('[class*="activity"], [class*="log"], tr').forEach(el => {
    ...
    console.log('  Sample activity items: ' + JSON.stringify(activityItems.slice(0, 3)) + '\n');
    ...
    console.log('TEST 8: Check Console for Errors');
    const consoleErrors = [];
    page.on('console', msg => {
      if (msg.type() === 'error') {
    ...
    console.log('TEST 9: Verify Activity Log Table Structure');
