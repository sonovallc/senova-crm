...
Step 5: Check result');
    ...
    await page.screenshot({ path: path.join(dir, '06-final-state.png'), fullPage: true });
    ...
    if (errors > 0) {
      for (let i = 0; i < errors; i++) {
        const txt = await page.locator('.notification.is-danger, [role=alert]').nth(i).textContent();
        console.log('  ERROR:', txt);
        results.errors.push(txt);
      }
    }
    
    const urlOk = finalUrl.includes('dashboard') || finalUrl !== 'http://localhost:3004';
    results.success = urlOk && errors === 0;
    
    console.log('
========================================');
    console.log(results.success ? '✅ LOGIN PASSED' : '❌ LOGIN FAILED');
    console.log('  URL changed:', urlOk);
  ...
  console.log('
Results saved to screenshots/login-test/results.json');
}
