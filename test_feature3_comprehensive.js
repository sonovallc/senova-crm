...
(async () => {
  ...
  try {
    ...
    // TEST 1
    console.log('TEST 1: Templates Page');
    try {
      const link = page.locator('a[href*="/email/templates"], a:has-text("Templates")').first();
      if (await link.count() > 0) {
        await link.click();
        await page.waitForURL('**/email/templates**', { timeout: 5000 });
        await page.waitForLoadState('networkidle');
        await takeScreenshot(page, getScreenshotFilename('01-templates-page-loaded'));
        ...
        const test1Pass = foundTemplates === 10 && hasNewButton;
        
        recordTest('TEST 1: Templates Page', test1Pass ? 'PASS' : 'FAIL', { found: foundTemplates });
        console.log('TEST 1: ' + (test1Pass ? 'PASS' : 'FAIL'));
      } else {
        recordTest('TEST 1: Templates Page', 'FAIL', { error: 'Link not found' });
      }
    } catch (error) {
      recordTest('TEST 1: Templates Page', 'FAIL', { error: error.message });
    }
