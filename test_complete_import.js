...
(async () => {
  ...
  try {
    ...
    console.log('');
    console.log('=== CONTINUING TO COMPLETE WORKFLOW ===');
    
    console.log('\nSTEP 12: CLICK NEXT TO GO TO TAGS');
    const nextBtn1 = page.locator('button:has-text("Next")').first();
    await nextBtn1.click();
    await page.waitForTimeout(3000);
    await page.screenshot({ path: 'screenshots/full-complete-03-tags.png', fullPage: true });
    console.log('Success: On Tags step');
    
    console.log('\nSTEP 13: CLICK NEXT TO GO TO DUPLICATES');
    const nextBtn2 = page.locator('button:has-text("Next")').first();
    await nextBtn2.click();
    await page.waitForTimeout(3000);
    await page.screenshot({ path: 'screenshots/full-complete-04-duplicates.png', fullPage: true });
    console.log('Success: On Duplicates step');
    
    console.log('\nSTEP 14: SELECT SKIP & GO TO PREVIEW');
    const radios = await page.$$('input[type="radio"]');
    if (radios.length > 0) await radios[0].click();
    const nextBtn3 = page.locator('button:has-text("Next")').first();
    await nextBtn3.click();
    await page.waitForTimeout(5000);
    await page.screenshot({ path: 'screenshots/full-complete-05-preview.png', fullPage: true });
    ...
    await page.waitForTimeout(2000);
    await page.screenshot({ path: 'screenshots/full-complete-06-importing.png', fullPage: true });
    console.log('Success: Import started');
    
    console.log('\nSTEP 16: WAIT FOR COMPLETION (90s)');
    await page.waitForTimeout(90000);
    await page.screenshot({ path: 'screenshots/full-complete-07-done.png', fullPage: true });
    console.log('Success: Import complete');
    
    console.log('\nSTEP 17: VERIFY CONTACTS');
    const closeBtn = page.locator('button:has-text("Close"), button:has-text("Done")').first();
    if (await closeBtn.count() > 0) await closeBtn.click();
    await page.waitForTimeout(3000);
    await page.goto('http://localhost:3004/dashboard/contacts', { waitUntil: 'networkidle' });
    await page.waitForTimeout(3000);
    await page.screenshot({ path: 'screenshots/full-complete-08-contacts.png', fullPage: true });
    console.log('Success: Back to contacts list');


  } catch (error) {
    console.error('\nERROR:', error.message);
    await page.screenshot({ path: 'screenshots/full-complete-ERROR.png', fullPage: true });
    throw error;
  } finally {
    await browser.close();
  }
