...
async function testArchiveButton() {
  ...
  try {
    ...
    await page.waitForSelector('.bg-white.border.rounded', { timeout: 10000 });
    
    // Get all thread items (the cards with contact info)
    const threadCards = await page.locator('.bg-white.border.rounded').all();
    ...
    const threadsAfter = await page.locator('.bg-white.border.rounded').all();
    ...
    const archivedThreads = await page.locator('.bg-white.border.rounded').all();
