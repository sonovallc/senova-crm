...
(async () => {
  ...
  try {
    ...
    console.log('CLICKED in ' + clickTime + 'ms');
    
    await page.screenshot({ path: path.join(screenshotDir, '06-clicked.png'), fullPage: true });
    
    console.log('STEP 5: Verify Success');
    
    await page.waitForTimeout(3000);
    await page.screenshot({ path: path.join(screenshotDir, '07-after-click.png'), fullPage: true });
    
    await page.waitForTimeout(2000);
    await page.screenshot({ path: path.join(screenshotDir, '08-final.png'), fullPage: true });

    console.log('STEP 6: Check Errors');
    ...
    if (clickTime < 1000 && errors.length === 0) {
      console.log('STATUS: VERIFIED FIXED');
