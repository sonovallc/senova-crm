#!/usr/bin/env python3
"""
Seed script for Senova CRM initial data - LOCAL VERSION.
Creates master owner profile and Senova CRM object.

This version is designed to work with local Docker PostgreSQL.

Run with: python scripts/seed_senova_data_local.py
"""
...
async def seed_data():
    ...
    logger.info("="*60)
    logger.info("Starting Senova CRM data seeding (LOCAL VERSION)...")
    logger.info(f"Database: {DATABASE_URL.split('@')[1]}")  # Show connection info without password
    logger.info("="*60 + "\n")
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            if success:
                ...
                logger.info("\n📝 Login credentials:")
                logger.info(f"   Email: {MASTER_OWNER_EMAIL}")
                logger.info(f"   Password: D3n1w3n1!")
