...
async function safeNavigate(page, url, name) {
  try {
    ...
    results.consoleErrors.push({
      text: `Navigation failed for ${name}: ${error.message}`,
      timestamp: new Date().toISOString()
    });
    return false;
  }
...
async function runFinalTest() {
  ...
  page.on('requestfailed', request => {
    ...
    // Don't log aborted requests as errors (common during navigation)
  ...
  try {
    console.log('\n=== PHASE 1: LOGIN ===');
    const loginSuccess = await safeNavigate(page, `${BASE_URL}/login`, 'Login');
    if (!loginSuccess) {
    ...
    if (await safeNavigate(page, `${BASE_URL}/dashboard/email/campaigns`, 'Campaigns')) {
      await page.waitForTimeout(4000); // Extended wait for data loading
      ...
      const campaignsContent = await page.locator('table, .campaign-item, text=Create New Campaign, button:has-text("Create")').count();
      ...
      if (!campaignsLoading && campaignsContent > 0) {
        ...
        results.bugVerification['BUG-CAMPAIGNS-LOADING'].details = `No loading indicator, found ${campaignsContent} content elements`;
        console.log(`⚠️ BUG-CAMPAIGNS-LOADING: INDETERMINATE (${campaignsContent} elements found)`);
      }
    }
    ...
    if (await safeNavigate(page, `${BASE_URL}/dashboard/email/autoresponders`, 'Autoresponders')) {
    ...
    if (await safeNavigate(page, `${BASE_URL}/dashboard/email/campaigns/create`, 'Campaigns Create')) {
      await page.waitForTimeout(4000);
    ...
    if (await safeNavigate(page, `${BASE_URL}/dashboard/email/compose`, 'Compose')) {
      await page.waitForTimeout(2000);
    ...
    if (await safeNavigate(page, `${BASE_URL}/dashboard/email/templates`, 'Templates')) {
      await page.waitForTimeout(2000);
    ...
    if (await safeNavigate(page, `${BASE_URL}/dashboard/settings/integrations/mailgun`, 'Mailgun Settings')) {
      await page.waitForTimeout(2000);
    ...
    if (await safeNavigate(page, `${BASE_URL}/dashboard/email/inbox`, 'Inbox')) {
      await page.waitForTimeout(2000);
  ...
  const nonAbortedErrors = results.consoleErrors.filter(e => !e.text.includes('ERR_ABORTED'));
  console.log(`   Total Errors: ${nonAbortedErrors.length} (excluding navigation aborts)`);
  if (nonAbortedErrors.length > 10) {
    console.log('   (Showing first 10 errors)');
    nonAbortedErrors.slice(0, 10).forEach((err, i) => {
      console.log(`   ${i + 1}. ${err.text}`);
    });
  } else if (nonAbortedErrors.length > 0) {
    nonAbortedErrors.forEach((err, i) => {
      console.log(`   ${i + 1}. ${err.text}`);
    });
  } else {
  ...
  const isProductionReady =
    ...
    passRate >= 90 &&
    bugFixRate >= 66 && // At least 2 out of 3 bugs fixed
    results.bugVerification['BUG-CAMPAIGNS-LOADING']?.status === 'FIXED';
  ...
  if (isProductionReady) {
    ...
    if (bugFixRate < 66) {
      console.log(`   ✗ Bug fix rate: ${bugFixRate}% (need 66%)`);
    }
