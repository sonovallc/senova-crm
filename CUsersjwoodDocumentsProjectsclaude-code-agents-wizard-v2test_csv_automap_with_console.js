...
(async () => {
  ...
  try {
    ...
    await page.waitForSelector('text=Map CSV Columns', { timeout: 90000 });
    await page.waitForTimeout(3000);
    await page.screenshot({ path: 'screenshots/v3-import-test-04-step2.png', fullPage: true });
    console.log('✓ File processing complete, on mapping step');
    ...
    console.log('✓ Clicked Auto-Map Columns button');
    ...
    console.log('✓ Full console logs saved to screenshots/v3-import-console-logs.json');
    ...
    console.log('\n=== TEST RESULT ===');
    if (hasAutoMapLogs) {
      console.log('✓ Auto-map function was called (console logs detected)');
    } else {
      console.log('✗ Auto-map function may NOT have been called (no console logs)');
    }

    if (hasDropdownChanges) {
      console.log('✓ Dropdowns are present on the page');
    } else {
      console.log('✗ No dropdowns found on the page');
    }
