...
(async () => {
  ...
  try {
    ...
    await page.fill('input[name="firstName"]', 'TestContactFinal');
    await page.fill('input[name="lastName"]', 'AutomatedV4');
    await page.fill('input[name="email"]', testEmail);
    await page.fill('input[name="phone"]', '+1234567890');
    await page.fill('input[name="company"]', 'Test Company Final');
    
    // Check current status value
    const statusValue = await page.inputValue('select[name="status"]');
    console.log('Status dropdown value: ' + statusValue);
    ...
    const toastFound = successToast > 0 ? 'YES' : 'NO';
    console.log('Success toast found: ' + toastFound);
    ...
    const inListFound = contactInList > 0 ? 'YES' : 'NO';
    console.log('Contact in list: ' + inListFound);
    
    await page.screenshot({ path: 'screenshots/test1_v4_06_verified.png', fullPage: true });
    
    if (successToast > 0 || contactInList > 0) {
      console.log('✓ Contact verified');
      console.log('\n=== TEST RESULT: PASS ===');
    } else {
      console.log('✗ Contact NOT verified');
      console.log('\n=== TEST RESULT: FAIL ===');
    }
